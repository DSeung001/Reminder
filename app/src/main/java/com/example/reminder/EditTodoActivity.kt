        ...
        binding.btnDelete.setOnClickListener {
            ...
            builder.setTitle("해당 일을 정말 삭제하겠습니까?")
                ...
                DialogInterface.OnClickListener{
                    dialog, id ->
                    val intent = Intent().apply {
                        val todo = Todo(todo!!.id, todo!!.title, todo!!.started_at, todo!!.repeat, todo!!.content, todo!!.delay, todo!!.created_at, SimpleDateFormat("yyyy-MM-dd").format(System.currentTimeMillis()),)
                        putExtra("todo", todo)
                        putExtra("flag", 2)
                    }
                    setResult(RESULT_OK, intent)
